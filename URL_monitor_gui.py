...
class Monitor_Gui:
    ...
    def __init__(self, master):
        ...
        ttk.Radiobutton(frame1, text='1 minute', variable=self.period,
                                value=60).pack(anchor='w')
        ttk.Radiobutton(frame1, text='5 minutes', variable=self.period,
        ...
        ttk.Radiobutton(frame1, text='5 seconds', variable=self.timeout,
                                value=5).pack(anchor='w')
        self.timeout.set(1)
    ...
    def stop(self):
        '''Clear output textbox, set monitoring flag to False and toggle button state
        '''
    ...
    def toggle_button(self):
        ...
        if self.bstate.get() == 'Start Monitoring':
            ...
            self.start_button.config(text='Stop Monitoring', command=self.stop)
            self.s.configure('TButton', foreground='red', relief='raised',
                                        padding=5, state=ACTIVE)
        else:
            self.bstate.set('Start Monitoring')
            self.start_button.config(text='Start Monitoring', command=self.start)
            self.s.configure('TButton', foreground='green', relief='sunken',
                                        padding=5, state=DISABLED)
